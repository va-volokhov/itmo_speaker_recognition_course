...
def framing(emphasized_signal, sample_rate=16000, frame_size=0.025, frame_stride=0.01):
    ...
    """
    Performs framing of the input signal emphasized_signal with sample rate sample_rate
...
def compute_fbank_filters(nfilt=40, sample_rate=16000, NFFT=512):
    ...
    """
    Computes fbank filters matrix for special case (sample_rate & NFFT)
    ...
    ###########################################################
    # Here is your code to convert Convert Hz to Mel: high_freq -> high_freq_mel
    ...
    ###########################################################
    # Here is your code to convert Convert Mel to Hz: mel_points -> hz_points
...
def compute_mfcc(filter_banks_features, num_ceps=20):
    ...
    :return: mfcc
    """
    mfcc = dct(filter_banks_features, type=2, axis=1, norm='ortho')[:, 1 : (num_ceps + 1)] # Keep 2-end
...
def cmvn_floating(features, LC, RC, unbiased=False):
    ...
    """
        ...
        features is the feature matrix (nframes x dim)
        LC, RC is  and
