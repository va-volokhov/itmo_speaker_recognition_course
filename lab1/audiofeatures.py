...
import numpy as np
from typing import Tuple, Iterable, Callable, Optional, Dict, List
...
fbank_mx      = mel_fbank_mx(winlen_nfft = WINDOWSIZE/SOURCERATE,
                             fs          = fs,
                             NUMCHANS    = NUMCHANS,
                             LOFREQ      = LOFREQ,
                             HIFREQ      = HIFREQ)
...
class Audiofeatures():
    ...
    def extract(self, wav_path: Path) -> None:
        """
        :return:
        """
    ...
    def preemphasis(self, waveform: np.ndarray) -> np.ndarray:
        """
        ...
        x = framing(waveform.astype("float"), window.size, int(window.size - noverlap)).copy()
    ...
    def _extract_filterbanks(self, waveform: np.ndarray) -> np.ndarray:
        """
    ...
    def _dct(self, waveform: np.ndarray) -> np.ndarray:
        """
    ...
    def _mvn(self, feats: np.ndarray) -> np.ndarray:
        """
...
def framing(a, window, shift=1):
    shape = (int((a.shape[0] - window) / shift + 1), int(window)) + a.shape[1:]
    strides = (a.strides[0]*int(shift), a.strides[0]) + a.strides[1:]
...
def mfcc_htk(x, window, noverlap, fbank_mx, nfft=None,
    ...
    x = framing(x.astype("float"), window.size, int(window.size-noverlap)).copy()
    if ZMEANSOURCE:
